...
要求：[BepInEx 插件管理器](https://github.com/BepInEx/BepInEx) 5.4.21 或更高版本（请先按照其安装说明操作！）。

#### Unity 2022+ 特别说明

对于 Unity 2022 或更高版本，请确保：

1. 使用 BepInEx 6.0 或更高版本
2. 安装 IL2CPP 运行时（如果使用 IL2CPP 构建）
3. 从发布页面下载 `XUnity.AutoTranslator-BepInEx-IL2CPP` 包（如果使用 IL2CPP 构建）
4. 从[发布页](../../releases)下载 XUnity.AutoTranslator-BepInEx-{版本号}.zip。
5. 直接解压到游戏目录，使插件 DLL 文件放在 BepInEx 文件夹中。
6. 启动游戏。
7. 出于多种考虑，并非所有文本钩子都默认启用，因此如果您发现游戏或游戏的某些部分没有被正确翻译，不妨进入配置文件，启用一些已禁用的文本框架！配置文件会在游戏启动时创建。
...
我们完全重写了翻译引擎以支持 Unity 2022 及更高版本，包括：
...
1. 删除 `BepInEx/config/BepInEx.cfg` 并重新启动
2. 检查游戏日志以获取错误信息
3. 确保没有冲突的模组
...
publicstaticclassTranslationRegistry
...
publicinterfaceITranslationRegistry
    ...
    /// <paramname="assembly">行为应应用于的程序集。</param>

    /// <paramname="package">包含翻译的包。</param>
