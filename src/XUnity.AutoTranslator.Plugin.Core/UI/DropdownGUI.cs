       ...
       where TSelection : class
    {
       ...
       private void ShowDropdownIL2CPP( float x, float y, float width, GUIStyle buttonStyle )
       {
          try
          {
             ...
             // For IL2CPP, dynamically calculate height to show more options
             float maxAllowedHeight = Mathf.Min( totalHeight, Screen.height - y - 50 ); // Don't go off screen
             float dropdownHeight = Mathf.Min( totalHeight, maxAllowedHeight );
             dropdownHeight = Mathf.Max( dropdownHeight, MaxHeight ); // Ensure minimum height

             // Create a simple box background
             GUI.Box( GUIUtil.R( x, y, width, dropdownHeight ), "", GUIUtil.NoSpacingBoxStyle );
             ...
             // Options - show as many as can fit
             foreach( var option in options )
             {
                if( currentY + GUIUtil.RowHeight > y + dropdownHeight )
                {
                   break; // No more space
                }
